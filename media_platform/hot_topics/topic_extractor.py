...
class TopicExtractor:
    ...
    def __init__(self):
        """初始化话题提取器"""
        self.llm = ChatDeepSeek(model=DS_MODEL_CHAT, api_key=DS_API_KEY)
    ...
    def extract_keywords_and_summary(
        ...
        try:
            ...
            fallback_summary = (
                f"今日共收集到 {len(news_list)} 条热点新闻，涵盖多个平台的热门话题。"
            )
    ...
    def _build_news_summary(self, news_list: List[Dict]) -> str:
        """
        构建新闻摘要文本
        目前只使用了新闻的title和source_platform作为分析源
        TODO: 使用新闻内容，提取更多的关键词
        """
        news_items = []
...
"""
    ...
    def _manual_parse_result(self, text: str) -> Tuple[List[str], str]:
        ...
        for line in lines:
            ...
            # 如果这一行看起来像总结内容
                ...
                "今日" in line or "热点" in line or "新闻" in line
            ):
                if not summary:
                    summary = line
    ...
    def get_search_keywords(self, keywords: List[str], limit: int = 10) -> List[str]:
        ...
        for keyword in keywords:
            ...
            if (
                len(keyword) > 1
                and len(keyword) < 20  # 不能太长
                and keyword not in search_keywords
                and not keyword.isdigit()  # 不是纯数字
                and not re.match(r"^[a-zA-Z]+$", keyword)
            ):  # 不是纯英文（除非是专有名词）
