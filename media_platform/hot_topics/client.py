...
class HotTopicsExtraction:
    """HotTopicsExtraction主要工作流程"""
    ...
    def __init__(self):
        ...
        logger.info("HotTopicsExtraction 初始化完成")
    ...
    async def run_daily_extraction(
        self, news_sources: Optional[List[str]] = None, max_keywords: int = 100
    ) -> Dict:
        """
        运行每日话题提取流程
        ...
        try:
            ...
            extraction_result["database_save"] = {"success": save_success}
...
async def run_extraction_command(sources=None, keywords_count=100, show_details=True):
    ...
    try:
        async with HotTopicsExtraction() as extractor:
            # 运行话题提取
            result = await extractor.run_daily_extraction(
                news_sources=sources, max_keywords=keywords_count
            )
            ...
            if result["success"]:
                if show_details:
                    ...
                    logger.info(
                        f"   提取关键词: {len(topic_data.get('keywords', []))} 个"
                    )
                    logger.info(
                        f"   生成总结: {len(topic_data.get('summary', ''))} 字符"
                    )
...
def main():
    ...
    parser.add_argument(
        "--sources", nargs="+", help="指定新闻源平台", choices=list(SOURCE_NAMES.keys())
    )
    parser.add_argument(
        "--keywords", type=int, default=100, help="最大关键词数量 (默认100)"
    )
